...
export default function LifePage() {
  ...
  return <main className="life-page"><nav className="life-nav"><Link href="/" className="brand"><span>SB</span> SAAR BITES</Link><div><Link href="/">← {c.back}</Link><Link href="/reviews">{locale === 'en' ? 'Latest reviews' : locale === 'de' ? 'Neue Bewertungen' : '最新打卡'}</Link><Link href="/board">{locale === 'en' ? 'Message board' : locale === 'de' ? 'Pinnwand' : '留言板'}</Link><div className="life-languages" aria-label="Language">{(['mix','en','de'] as Lang[]).map(item => <button key={item} aria-pressed={lang===item} onClick={() => { setLang(item); localStorage.setItem('saar-bites-language',item); }}>{item === 'mix' ? '中文' : item.toUpperCase()}</button>)}</div></div></nav>
